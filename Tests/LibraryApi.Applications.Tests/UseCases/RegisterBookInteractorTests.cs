...
public class RegisterBookInteractorTests
{
    ...
    [TestMethod("正常時:構築・保存しコミットしてDTOを返す")]
    ...
    [TestMethod("分類が存在しなければInvalidInputExceptionをスローする")]
    public async Task ExecuteAsync_TestCase02()
    {
        ...
        await Assert.ThrowsExceptionAsync<InvalidInputException>(
    ...
    [TestMethod("保存失敗時:ロールバックして例外を再スローする")]
