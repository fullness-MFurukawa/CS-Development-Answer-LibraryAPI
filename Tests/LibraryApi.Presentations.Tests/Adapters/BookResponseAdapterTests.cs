...
public class BookResponseAdapterTests
{
    ...
    [TestMethod("Convert:BookDtoをBookResponseに変換する(入れ子の分類も変換される)")]
    ...
    [TestMethod("Restore:サポートしないためNotSupportedExceptionとなる")]
    public void Restore_TestCase01()
    {
        var adapter = CreateAdapter();
        var response = new BookResponse();

        Assert.ThrowsException<NotSupportedException>(() => adapter.Restore(response));
    }
}