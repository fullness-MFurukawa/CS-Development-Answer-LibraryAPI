...
public class BookStockAdapterTests
{
    ...
    [TestMethod("ドメインのBookStockをBookStockEntityに変換する")]
    ...
    [TestMethod("BookStockEntityをドメインのBookStockに復元する")]
    ...
    [TestMethod("復元時にドメインの検証が働きUUIDが空ならDomainExceptionをスローする")]
    public void Restore_TestCase2()
    {
        ...
        // Act / Assert
        Assert.ThrowsException<DomainException>(() => _adapter.Restore(entity));
    }
}